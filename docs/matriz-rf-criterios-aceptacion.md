...
| RF-05 | Notificacion de visita al cliente por correo. | La UI registra notificacion simulada y confirma acciones. Variables SMTP estan preparadas. Falta envio real SMTP y plantillas para produccion. | Listo demo / Parcial produccion |
...
| RF-09 | Confirmacion o rechazo del cliente mediante enlaces unicos. | API incluye endpoints publicos base con token. La UI permite simular confirmacion o rechazo del cliente y actualiza estado/resultado. Falta correo real con enlaces y expiracion productiva. | Listo demo / Parcial produccion |
...
| 13 | El cliente puede confirmar o rechazar nueva fecha mediante enlace, si se implementa. | Se simula confirmacion/rechazo desde la UI y existen endpoints publicos base; falta enlace real por correo. | Listo demo / Parcial produccion |
...
- **Listo para prueba demo**: login/registro, agenda 5 dias, toma de hora, no RUT, doble reserva en UI, vista admin, bloqueo con motivo, reagendamiento, asistencia/no asistencia, resultado libre, seguimiento, confirmaciones Si/No, simulacion de confirmacion/rechazo cliente y outbound.
- **Parcial tecnico**: SMTP real, enlaces publicos por correo, filtros avanzados, conexion frontend-backend productiva, PostgreSQL transaccional.
